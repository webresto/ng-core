...
import { DishChildModifier, DishListItem, DishModifier, NgRestoCartService } from "../../services/ng-restocart.service";
...
export class DishCalcComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges() {
    ...
    if (this.dish && this.dish.modifiers) {
      for (let modifier of this.dish.modifiers) {
        ...
        // Indexing
        let id = modifier.modifierId;
        this.modifiers.indexById[id] = modifier;
        ...
        switch (modifierType) {
          ...
          case 'custom':
            ...
            for (let childModifier of modifier.childModifiers) {
              // Indexing
              this.modifiers.indexById[childModifier.modifierId] = <DishModifier><unknown>childModifier;
              // Init default value
  ...
  calculateTotalAmountInGroup(groupId:string) {
    if (groupId !== 'single') {
    this.modifiers.indexById[groupId].totalAmount = (<number>Object
      .values(this.modifiersValueTree[groupId])
      .reduce((a: any, b: any) => a + b));
    return this.modifiers.indexById[groupId].totalAmount;} else {
      return undefined;
    }
  }

  checkImagesInModifier(modifierId:string) {
  ...
  getModifiersIds(modifier: { dish: { groupId: any; }; modifierId: any; }) {
    return {
  ...
  selectTwoPartsAssembledModifier(modifier: any) {
    ...
    if (amount && !this.twoPartsAssembledModifiersIdsByGroupId[groupId].find((v: any) => v == modifierId)) {
